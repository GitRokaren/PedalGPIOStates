...
${On}=     ${0} 
${Off}=    ${1}
#The numbers are reversed due to the GPIOzero logic, 
#where input value is reversed to the voltage level
...
StartTransfer
    [Arguments]    ${PinReady}
    Turn On Pin    ${PinReady}
#this is the pin that will tell the other Pi 
#to get ready for transfer
    
StopTransfer
    [Arguments]     ${PinReady}
    Turn Off Pin    ${PinReady}
#This function is used to "reset" the Pins so that 
#they're ready to send the next
...
CheckStates
    ...
    [Return]                          ${Result}
#This function is used to check if the set-state corresponds 
#with the state that the ctrl panel says it's in 
...
CloseInPins
    ...
    END
#These functions are used to turn off the pins in use, to prevent damage of hardware